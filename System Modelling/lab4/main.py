...
# classes used for collecting metrics
@dataclass(eq=False)
...
def create_model(num_nodes: int, factory_time: float, queueing_time: float,
    ...
    num_digits = len(str(2 * num_nodes)) # this is *2 because we have a transition node between each queueing node, used only for naming
    ...
    node_idx = 2 # also used for naming
    for idx in range(num_nodes): # we count only queueing nodes as requested by parameter num_nodes
        ...
        node_idx += 1
        if idx > 1: # we can't add a transition node for the first queueing node because the previous node is factory node
            node = ProbaTransitionNode[Item, NodeMetrics](name=f'{node_idx:0{num_digits}d}_previous_vs_next',
                                                          metrics=NodeMetrics())
            node_idx += 1
            # For a transition node, prev_node is the node from which we are trying to leave. 
            # we are going to prev.prev because we are right now looking from position of a transition node. 
...
if __name__ == '__main__':
    save_location = Path('/tmp/time_complexity.png')
    num_nodes = 20
    factory_time = 0.5
    queueing_time = 0.2
    prev_proba = 0.1
    ...
    figure.savefig(save_location, dpi=300)
